...
public class AdminGUI extends JPanel
{
	...
	private final String adminpassord = "123";	//default passord
	private int søkFor;
	...
	private JButton søkeKnapp, nyAns, slettAns, deaktiverAns, godkjennKnapp, avslåKnapp, lagre, oppdater, visAnsattKnapp, statButton, visKunForsikring, resettKnapp;
	...
	public AdminGUI(Huvudvindu v)
	{
		...
		resettKnapp = new JButton("Resett passord");
		...
		resettKnapp.addActionListener(lytter);
	...
	public void resettPassord()
	{
		try{
			...
			if(tabbedPane.getSelectedIndex() == 0)
			{
				...
				anna = register.getAnsattViaNr( (String)tableModel.getValueAt(row, TModel.PERSON_NR) );
				/*String melding = "Ønsker du å resette denne brukerens passord til 'passord'?";
				String tittel = "Resetting av passord";
				int svar = JOptionPane.showConfirmDialog(null, melding, tittel, JOptionPane.YES_NO_OPTION);
				if(svar == JOptionPane.YES_OPTION)
				{
					register.resettPassord(anna, null);
					return;
				}*/
			}
			else if(tabbedPane.getSelectedIndex() == 1)
			{
				...
				kuun = register.getKundeViaNummer( (String)tableModel2.getValueAt(row, TModel.PERSON_NR) );
			}
			else
				return;
			...
			if(svar == JOptionPane.YES_OPTION)
			{
				register.resettPassord(anna, kuun);
				return;
			}
			else
				return;
	...
	public boolean passordSjekk(String p)
	{
		if(p == adminpassord)
			return true;
		else
	...
	private class Lytterklasse implements ActionListener//knappelytter
	{
		public void actionPerformed(ActionEvent e)
		{
				...
				|| e.getSource() == PersNr)
			{
				...
				visStatistikk();
			else if(e.getSource() == resettKnapp)
				resettPassord();
