...
public class Register implements Serializable
{
	...
	private final String respass = "passord";
	...
	//TODO gjør disse 4 til 1 generell funksjon for alle forsikringer!
	...
	public BilForsikring getBilViaRegNr(String nr)		//via personnummeret
	{
		String mnr = nr;
		BilForsikringsReg aReg = bilReg.finnBilViaRegNr(mnr);
		if(aReg == null)
			return null;
		Iterator<BilForsikring> iter = aReg.iterator();
		return iter.next();
	}
	...
	public BåtForsikring getBåtViaRegNr(String nr)		//via personnummeret
	{
		String mnr = nr;
		BåtForsikringsReg aReg = båtReg.finnBåtViaRegNr(mnr);
		if(aReg == null)
			return null;
		Iterator<BåtForsikring> iter = aReg.iterator();
		return iter.next();
	}
	...
	public HusForsikring getHusViaAdresse(String adr)		//via personnummeret
	{
		String mnr = adr;
		HusForsikringsReg aReg = husReg.finnHusViaAdresse(mnr);
		if(aReg == null)
			return null;
		Iterator<HusForsikring> iter = aReg.iterator();
		return iter.next();
	}
	...
	public HytteForsikring getHytteViaAdresse(String adr)		//via personnummeret
	{
		String mnr = adr;
		HytteForsikringsReg aReg = hytReg.finnHytteViaAdresse(mnr);
		if(aReg == null)
			return null;
		Iterator<HytteForsikring> iter = aReg.iterator();
		return iter.next();
	}
	...
	public void resettPassord(Ansatt anna, Kunde kunn)
	{
		if(!(anna == null))
		{
			anna.setPassord(respass);
			anna.setResatt(true);
		}
		else if(!(kunn == null))
		{
			kunn.setPassord(respass);
			kunn.setResatt(true);
		}
		else
		{
